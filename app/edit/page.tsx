...
export default function ResumeLatexPage() {
  ...
  useEffect(() => {
    ...
    const defaultName = `Roshan_Kumar_Sahu_${selectedRole
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 pt-4 border-t border-border">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Upload compiled PDF to Google Drive (optional)
              </h3>
              <p className="text-[11px] text-muted-foreground">
                After you compile this LaTeX to a PDF, select the PDF file here
                and upload it. The file will be stored in the Drive folder
                configured for the <span className="font-medium">{selectedRole}</span>{" "}
                role, and made publicly sharable.
              </p>
              <div className="flex flex-col gap-2 text-xs">
                <label className="flex flex-col gap-1">
                  <span className="text-[11px] font-medium text-muted-foreground">
                    Desired file name on Drive
                  </span>
                  <input
                    type="text"
                    value={uploadFileName}
                    onChange={(e) => setUploadFileName(e.target.value)}
                    className="w-full rounded-md border border-border bg-background/80 px-2 py-1"
                    placeholder="Roshan_Kumar_Sahu_FSD_2025.pdf"
                  />
                </label>
                <input
                  type="file"
                ...
                <button
                  type="button"
                  className="inline-flex items-center justify-center rounded-md border border-border bg-background/80 px-3 py-1.5 text-xs hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed"
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (!response.ok) {
                        ...
                        setUploadMessage(
                          body?.error ||
                            "Upload failed. Check server logs or Drive config.",
                        )
                        return
                      }
                      setUploadMessage(
                        body?.webViewLink ||
                          "Upload successful. Public link created.",
                      )
                    } catch (error) {
                      console.error("Error uploading PDF:", error)
                      setUploadMessage(
                        "Unexpected error while uploading PDF to Drive.",
                      )
                    } finally {
                      setUploading(false)
                    }
